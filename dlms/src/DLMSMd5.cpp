...
void DLMSMD5::Encode(unsigned char * output, unsigned int *input, unsigned int len)
{
    unsigned int pos = 0;
...
void DLMSMD5::Transform(unsigned char* block, unsigned int *state)
{
...
int DLMSMD5::Update(unsigned char* data, unsigned int len, unsigned char* buffer, unsigned int* count, unsigned int* state)
{
    unsigned int i;
    // Number of bytes.
...
int DLMSMD5::Encrypt(ByteBuffer& data, ByteBuffer& digest)
{
