...
short DateTime::GetCurrentTimeZone()
{
    int hours, minutes, deviation;
...
char DateTime::GetCurrentDeviation()
{
    int hours, minutes, deviation;
    GetUtcOffset(NULL, hours, minutes, deviation);
    return deviation;
...
DateTime::DateTime(const unsigned long unixTime)
{
    m_Deviation = 0;
    time_t t = unixTime;
    m_Value = *gmtime(&t);
...
DateTime::DateTime(const unsigned long long unixTime)
{
    m_Deviation = 0;
    time_t t = unixTime;
    m_Value = *gmtime(&t);
...
// Constructor.
DateTime::DateTime(int year, int month, int day, int hour, int minute, int second, int millisecond, int devitation)
{
    Init(year, month, day, hour, minute, second, millisecond, devitation);
}

void DateTime::GetSystemDateTimeFormat(std::string& value)
{
    char buff[50];
    struct tm order = { 0 };
    order.tm_year = 0;
...
int DateTime::GetDateFormat2(GXDLMS_DATE_FORMAT& format, char& separator)
{
...
int DateTime::GetTimeFormat2(char& separator, char& use24HourClock)
{
    int ret = 0, pos;
    char buff[15];
    char pm[15];
...
int DateTime::GetDateTimeFormat(
    std::string& value,
...
int Remove(DateTime* value, std::string& format, char dateSeparator, char timeSeparator)
{
    if (dynamic_cast<Date*>(value) != NULL)
    {
    ...
    else if (dynamic_cast<Time*>(value) != NULL)
    {
...
int DateTime::FromString(const char* datetime)
{
    int ret;
    ...
    if ((ret = GetDateTimeFormat(format, df, dateSeparator, timeSeparator, use24HourClock)) == 0)
    {
        ...
        char g[15];
        g[0] = 0;
        if (dynamic_cast<Date*>(this) != NULL)
        {
#if _MSC_VER > 1000
        ...
        else if (dynamic_cast<Time*>(this) != NULL)
        {
...
int DateTime::ToFormatString(const char* pattern, std::string& value)
{
    int ret;
    char buff[30];
    value.clear();
    if (m_Skip != 0 || m_Extra != 0)
    {
            ...
            (ret = GetDateTimeFormat(format, df, dateSeparator, timeSeparator, use24HourClock)) == 0)
        {
            ...
            ret = (int)strftime(buff, sizeof(buff), Helpers::trim(format).c_str(), &m_Value);
            if (ret != 0)
            {
                ret = 0;
                value = buff;
...
int DateTime::ToFormatString(std::string& value)
{
    return ToFormatString(NULL, value);
}

// Constructor.
void DateTime::Init(int year, int month, int day, int hour, int minute, int second, int millisecond, int devitation)
{
    memset(&m_Value, 0, sizeof(m_Value));
    m_Extra = DATE_TIME_EXTRA_INFO_NONE;
    m_Status = DLMS_CLOCK_STATUS_OK;
...
struct tm& DateTime::GetValue()
{
    return m_Value;
}

unsigned char DateTime::DaysInMonth(int year, short month)
{
...
void DateTime::SetValue(const struct tm& value)
{
    m_Value = value;
}

// Skip selected date time fields.
DATETIME_SKIPS DateTime::GetSkip()
{
    return m_Skip;
}

void DateTime::SetSkip(DATETIME_SKIPS value)
{
    m_Skip = value;
}

DATE_TIME_EXTRA_INFO DateTime::GetExtra()
{
    return m_Extra;
}

void DateTime::SetExtra(DATE_TIME_EXTRA_INFO value)
{
    m_Extra = value;
}

int DateTime::GetDeviation()
{
    return m_Deviation;
}

void DateTime::SetDeviation(int value)
{
    m_Deviation = value;
}
...
// Status of the clock.
DLMS_CLOCK_STATUS DateTime::GetStatus()
{
    return m_Status;
}

void DateTime::SetStatus(DLMS_CLOCK_STATUS value)
{
    m_Status = value;
}
...
int DateTime::AddHours(int hours)
{
...
int DateTime::AddMinutes(int minutes)
{
...
int DateTime::AddSeconds(int seconds)
{
...
int DateTime::CompareTo(DateTime& antherDate)
{
...
int DateTime::ToLocalTime(struct tm& localTime)
{
    localTime = m_Value;
...
long DateTime::GetDifference(struct tm& start, DateTime& to)
{
    long diff = 0;
    // Compare seconds.
...
unsigned long DateTime::ToUnixTime()
{
