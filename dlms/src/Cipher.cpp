...
void Cipher::shift_right_block(unsigned char* v)
{
...
void Cipher::MultiplyH(const unsigned char* x, const unsigned char* y, unsigned char* z)
{
...
void Cipher::Inc32(unsigned char* block)
{
    unsigned long val;
...
void Cipher::Gctr(unsigned int* aes, const unsigned char* icb, unsigned char* in, int len, unsigned char* out)
{
    size_t i, n, last;
...
void Cipher::AesGcmGctr(unsigned int* aes, const unsigned char* J0, unsigned char* in, int len, unsigned char* out)
{
...
void Cipher::AesGcmGhash(const unsigned char* H, const unsigned char* aad, int aad_len,
    ...
    bool encrypt)
{
    ...
    int secret_size = secret.GetSize();

    //Must be either 8 byte system title or 16 byte authentication key
    if (secret_size != 8 && secret_size != 16)
    {
        return DLMS_ERROR_CODE_INVALID_PARAMETER;
    }
    ...
    if (secret_size == 8)
    {
        if ((ret = GetNonse(frameCounter, secret, nonse)) != 0)
        {
            return ret;
        }
    }

    if ((ret = Int(aes, key.m_Data, 16 * secret_size)) != 0)
    {
        return ret;
    }
    aes[60] = 10;
    ...
    if (encrypt && type == DLMS_COUNT_TYPE_PACKET)
    {
        ...
        if ((ret = nonse.SetUInt8(tag)) == 0)
        {
            ...
            {
                Helpers::SetObjectCount(secret_size, nonse);
                nonse.Set(secret.GetData(), secret_size);
            }

            Helpers::SetObjectCount(5 + input.GetSize(), nonse);

            if ((ret = nonse.SetUInt8(security)) == 0 &&  (ret = nonse.SetUInt32(frameCounter)) == 0)
            {
    ...
    uint64_t& InvocationCounter)
{
    ...
    unsigned long frameCounter;
    DLMS_COMMAND cmd;
    ByteBuffer* pTitle = &title;
    ByteBuffer systemTitle, countTag;
    ...
    case DLMS_COMMAND_GENERAL_DED_CIPHERING:
        if ((ret = Helpers::GetObjectCount(data, length)) != 0)
        {
            return ret;
        }
        if (length != 0)
...
bool Cipher::IsCiphered()
{
    return m_Security != DLMS_SECURITY_NONE;
}

DLMS_SECURITY Cipher::GetSecurity()
{
    return m_Security;
}

void Cipher::SetSecurity(DLMS_SECURITY value)
{
    m_Security = value;
}
...
/**
    ...
    */
DLMS_SECURITY_SUITE Cipher::GetSecuritySuite()
{
 ...
 *            Used security suite.
 */
void Cipher::SetSecuritySuite(DLMS_SECURITY_SUITE value)
{
    m_SecuritySuite = value;
}

ByteBuffer& Cipher::GetSystemTitle()
{
    return m_SystemTitle;
}

void Cipher::SetSystemTitle(ByteBuffer& value)
{
    m_SystemTitle.Clear();
    m_SystemTitle.Set(value.m_Data, value.m_Size - value.m_Position);
}

ByteBuffer& Cipher::GetBlockCipherKey()
{
    return m_BlockCipherKey;
}

void Cipher::SetBlockCipherKey(ByteBuffer& value)
{
    m_BlockCipherKey.Clear();
    m_BlockCipherKey.Set(value.m_Data, value.m_Size - value.m_Position);
}

ByteBuffer& Cipher::GetAuthenticationKey()
{
    return m_AuthenticationKey;
}

void Cipher::SetAuthenticationKey(ByteBuffer& value)
{
    m_AuthenticationKey.Clear();
    m_AuthenticationKey.Set(value.m_Data, value.m_Size - value.m_Position);
}

unsigned long Cipher::GetFrameCounter()
{
    return m_FrameCounter;
}

void Cipher::SetFrameCounter(unsigned long value)
{
    m_FrameCounter = value;
}

unsigned long Cipher::GetInvocationCounter()
{
    return m_FrameCounter;
}

void Cipher::SetInvocationCounter(unsigned long value)
{
    m_FrameCounter = value;
}
...
ByteBuffer& Cipher::GetDedicatedKey()
{
    return m_DedicatedKey;
}

void Cipher::SetDedicatedKey(ByteBuffer& value)
{
