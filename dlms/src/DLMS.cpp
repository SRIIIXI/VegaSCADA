...
bool DLMS::UseHdlc(DLMS_INTERFACE_TYPE type)
{
...
bool DLMS::IsReplyMessage(DLMS_COMMAND cmd)
{
...
int DLMS::GetAddress(long value, unsigned long& address, int& size)
{
    if (value < 0x80)
    {
...
int DLMS::CheckInit(DLMSSettings& settings)
{
...
/////////////////////////////////////////////////////////////////////////////
int GetDataFromBlock(ByteBuffer& data, int index)
{
    ...
    ByteBuffer& reply)
{
    ...
    if ((data.GetMoreData() & DLMS_DATA_REQUEST_TYPES_FRAME) != 0)
    {
        ...
        {
            return DLMS::GetMacHdlcFrame(settings, id, 0, NULL, reply);
        }
        else
        {
            ret = DLMS::GetHdlcFrame(settings, id, NULL, reply);
        }
        return ret;
    }
    DLMS_COMMAND cmd = (DLMS_COMMAND)settings.GetCommand();
    std::vector<ByteBuffer> tmp;
    if (data.GetMoreData() == DLMS_DATA_REQUEST_TYPES_GBT)
    {
        DLMSLNParameters p(&settings, 0, DLMS_COMMAND_GENERAL_BLOCK_TRANSFER,
    ...
    else
    {
        ...
        if (settings.GetUseLogicalNameReferencing())
        {
            DLMSLNParameters p(&settings, 0, cmd,
    ...
    ByteBuffer& reply)
{
    ...
    int ret, len;
    ByteBuffer primaryAddress, secondaryAddress;
    if (settings.IsServer())
    {
    ...
    ByteBuffer& reply)
{
    ...
    val |= settings.GetPlcSettings().GetMacDestinationAddress() & 0xFFF;
    reply.SetUInt16(val);
    if ((ret = DLMS::GetHdlcFrame(settings, frame, data, tmp)) == 0)
    {
...
unsigned char GetInvokeIDPriority(DLMSSettings& settings, bool increase)
{
...
/**
     ...
     */
long GetLongInvokeIDPriority(DLMSSettings& settings)
{
    long value = 0;
    if (settings.GetPriority() == DLMS_PRIORITY_HIGH)
    {
...
/**
     ...
     */
void AddLLCBytes(DLMSSettings* settings, ByteBuffer& data)
{
    ByteBuffer tmp;
    tmp.Set(&data);
    data.Clear();
...
int Cipher0(DLMSLNParameters& p,
    ByteBuffer& reply)
{
    int ret;
    ByteBuffer tmp;
    ...
    ByteBuffer& title = p.GetSettings()->GetCipher()->GetSystemTitle();
    ...
    else
    {
            ...
            p.GetCommand() == DLMS_COMMAND_ACCESS_RESPONSE)
        {
            ...
            else
            {
                ...
                DLMSVariant tmp = *p.GetTime();
                if ((ret = Helpers::SetData(p.GetSettings(), reply, DLMS_DATA_TYPE_OCTET_STRING, tmp)) != 0)
                {
                    return ret;
                }
                if (p.GetCommand() != DLMS_COMMAND_EVENT_NOTIFICATION)
        ...
        if (p.IsMultipleBlocks() && (p.GetSettings()->GetNegotiatedConformance() & DLMS_CONFORMANCE_GENERAL_BLOCK_TRANSFER) == 0)
        {
            ...
            if (totalLength > p.GetSettings()->GetMaxPduSize())
            {
                ...
                len -= Helpers::GetObjectCountSizeInBytes(len);
            }
            Helpers::SetObjectCount(len, reply);
        ...
        if (p.GetCommand() == DLMS_COMMAND_GENERAL_BLOCK_TRANSFER || (p.IsMultipleBlocks() && (p.GetSettings()->GetNegotiatedConformance() & DLMS_CONFORMANCE_GENERAL_BLOCK_TRANSFER) != 0))
        {
            ...
            //Add data length.
            Helpers::SetObjectCount(bb.GetSize(), reply);
            reply.Set(&bb);
            if (p.GetCommand() != DLMS_COMMAND_GENERAL_BLOCK_TRANSFER)
            {
...
int DLMS::GetLnMessages(
    DLMSLNParameters& p,
    std::vector<ByteBuffer>& messages)
{
    int ret;
    ByteBuffer reply, tmp;
...
int DLMS::AppendMultipleSNBlocks(
    DLMSSNParameters& p,
    ByteBuffer& reply)
{
    ...
    {
        hSize += 1 + Helpers::GetObjectCountSizeInBytes(p.GetCount());
    }
    ...
    maxSize -= Helpers::GetObjectCountSizeInBytes(maxSize);
    ...
    {
        p.SetBlockIndex(p.GetBlockIndex() + 1);
        Helpers::SetObjectCount(p.GetCount(), reply);
    ...
    Helpers::SetObjectCount(maxSize, reply);
    return maxSize;
}
...
int DLMS::AddInvokeId(DLMSTranslatorStructure* xml, DLMS_COMMAND command, uint8_t type, uint32_t invokeId)
{
    if (xml != NULL)
    {
    ...
    ReplyData& data)
{
    ...
    if (data.GetXml() != NULL)
    {
        ...
        std::string str;
        DLMSTranslator::ErrorCodeToString(data.GetXml()->GetOutputType(), (DLMS_ERROR_CODE)ch, str);
    ...
    if (data.GetData().GetPosition() < data.GetData().GetSize())
    {
        ...
        if (data.GetXml() != NULL && (ch != 0 || data.GetData().GetPosition() < data.GetData().GetSize()))
        {
            ...
            {
                std::string str;
                DLMSTranslator::ErrorCodeToString(data.GetXml()->GetOutputType(), (DLMS_ERROR_CODE)type, str);
            ...
            else
            {
                ...
                if ((ret = Helpers::GetData(&settings, data.GetData(), di, value)) != 0)
                {
                    return ret;
                }
    ...
    if ((ret = reply.GetData().GetUInt8(&ch)) == 0)
    {
        ...
        if (reply.GetXml() != NULL)
        {
            if (reply.GetData().Available() != 0)
            {
                ...
                Helpers::GetObjectCount(reply.GetData(), blockLength);
...
int VerifyInvokeId(DLMSSettings& settings, ReplyData& reply)
{
    if (
#ifndef DLMS_IGNORE_XML_TRANSLATOR
        reply.GetXml() == NULL &&
    ...
    ReplyData& reply)
{
    ...
    if (ch != 0)
    {
        ...
        DLMSVariant val;
        if ((ret = DLMSClient::ChangeType(tmp, DLMS_DATA_TYPE_DATETIME, settings.GetUseUtc2NormalTime(), val)) != 0)
        {
            return ret;
        }
    ...
    if (reply.GetXml() != NULL)
    {
        ...
        if ((ret = Helpers::GetObjectCount(reply.GetData(), len)) != 0)
        {
            return ret;
        }
        ...
        for (unsigned long pos = 0; pos != len; ++pos)
        {
            ...
            DLMSTranslator::ErrorCodeToString(reply.GetXml()->GetOutputType(), (DLMS_ERROR_CODE)err, str);
...
/**
    ...
    */
int DLMS::HandleDataNotification(
    DLMSSettings& settings,
    ReplyData& reply)
{
    unsigned long invokeId;
    ...
    if (len != 0)
    {
        ...
        if ((ret = DLMSClient::ChangeType(tmp, DLMS_DATA_TYPE_DATETIME, t)) != 0)
        {
            return ret;
        }
    ...
    ReplyData& data)
{
    ...
    else if (type == DLMS_SET_RESPONSE_TYPE_LAST_DATA_BLOCK)
    {
        ...
        {
            // Result start tag.
            DLMSTranslator::ErrorCodeToString(data.GetXml()->GetOutputType(),
    ...
    {
        unsigned long cnt;
        if ((ret = Helpers::GetObjectCount(data.GetData(), cnt)) != 0)
        {
            return ret;
        }
#ifndef DLMS_IGNORE_XML_TRANSLATOR
...
int DLMS::HandleGbt(DLMSSettings& settings, ReplyData& data)
{
    int ret;
    unsigned char bc;
    unsigned long len;
    ...
    data.SetCommand(DLMS_COMMAND_NONE);
    if ((ret = Helpers::GetObjectCount(data.GetData(), len)) != 0)
    {
        return ret;
    }
    ...
    if (data.GetXml() != NULL)
    {
        if ((data.GetData().GetSize() - data.GetData().GetPosition()) != len)
        {
            ...
            str.append(Helpers::IntToString(len));
            str.append("and there are ");
            str.append(Helpers::IntToString(data.GetData().GetSize() - data.GetData().GetPosition()));
            str.append(" bytes.");
        ...
        {
            data.GetXml()->AppendComment("Last block: " + Helpers::IntToString(((bc & 0x80) != 0)));
            data.GetXml()->AppendComment("Streaming: " + Helpers::IntToString(data.GetStreaming()));
            data.GetXml()->AppendComment("Window size: " + Helpers::IntToString(windowSize));
        }
        std::string str;
...
int DLMS::HandleGloDedRequest(DLMSSettings& settings,
    ReplyData& data)
{
...
int DLMS::HandleGeneralCiphering(
    DLMSSettings& settings,
    ReplyData& data)
{
    unsigned char ch;
    ...
    ReplyData& data)
{
    ...
    if (cmd == DLMS_COMMAND_NONE)
    {
        ...
        case DLMS_COMMAND_ACCESS_REQUEST:
            ...
            {
                ret = DLMSLNCommandHandler::HandleAccessRequest(settings, NULL, data.GetData(), NULL, data.GetXml(), DLMS_COMMAND_NONE);
            }
            break;
...
int DLMS::GetData(DLMSSettings& settings,
    ByteBuffer& reply,
    ReplyData& data,
    ReplyData* notify)
{
    ReplyData* target = &data;
    int ret;
    unsigned char frame = 0;
    bool isLast = true;
    bool isNotify = false;
...
int DLMS::HandleGetResponseWithList(
    DLMSSettings& settings,
    ReplyData& reply)
{
    int ret;
...
int DLMS::HandleGetResponseNormal(
    DLMSSettings& settings,
    ReplyData& reply,
    bool& empty)
{
    ...
    else
    {
        ...
        if (reply.GetXml() != NULL)
        {
            ...
            {
                std::string str;
                DLMSTranslator::ErrorCodeToString(reply.GetXml()->GetOutputType(), (DLMS_ERROR_CODE)ch, str);
...
int DLMS::HandleGetResponseNextDataBlock(
    DLMSSettings& settings,
    ReplyData& reply,
    int index)
{
    ...
    // Get data size.
    Helpers::GetObjectCount(data, count);
    // if whole block is read.
    if ((reply.GetMoreData() & DLMS_DATA_REQUEST_TYPES_FRAME) == 0)
    {
        // Check Block length.
...
int DLMS::HandleWriteResponse(ReplyData& data)
{
    unsigned char ch;
    int ret;
    unsigned long count;
    if ((ret = Helpers::GetObjectCount(data.GetData(), count)) != 0)
    {
        return ret;
    }
...
int DLMS::ReadResponseDataBlockResult(
    DLMSSettings& settings,
    ReplyData& reply,
    int index)
{
    ...
    if ((ret = Helpers::GetObjectCount(reply.GetData(), blockLength)) != 0)
    {
        return ret;
    }
    // Is Last block.
    ...
    int index)
{
    ...
    if (first)
    {
        if ((ret = Helpers::GetObjectCount(reply.GetData(), cnt)) != 0)
        {
            return ret;
        }
    ...
    for (pos = 0; pos != cnt; ++pos)
    {
        ...
        case DLMS_SINGLE_READ_RESPONSE_DATA_ACCESS_ERROR:
            ...
            DLMSTranslator::ErrorCodeToString(
...
int DLMS::GetAddressBytes(unsigned long value, ByteBuffer& bytes)
{
    int ret;
    unsigned long address;
    int size;
...
int DLMS::GetValueFromData(DLMSSettings& settings, ReplyData& reply)
{
    int ret;
    DataInfo info;
    ...
    reply.GetData().SetPosition(reply.GetReadPosition());
    if ((ret = Helpers::GetData(&settings, reply.GetData(), info, value)) != 0)
    {
        return ret;
    }
    // If new data.
    if (value.vt != DLMS_DATA_TYPE_NONE)
    {
        ...
        else
        {
            if (value.Arr.size() != 0)
            {
                ...
                {
                    DLMSVariant tmp = reply.GetValue();
...
void DLMS::GetDataFromFrame(ByteBuffer& reply, ReplyData& info, bool hdlc)
{
...
void DLMS::GetLLCBytes(bool server, ByteBuffer& data)
{
    if (server)
    {
...
bool IsPlcSfskData(ByteBuffer& buff)
{
    if (buff.Available() < 2)
    {
        return false;
...
unsigned short DLMS::CountFCS16(ByteBuffer& buff, int index, int count)
{
    int ret;
    unsigned char ch;
...
uint32_t DLMS::CountFCS24(unsigned char* buff, int index, int count)
{
    unsigned char i, j;
    uint32_t crcreg = 0;
    for (j = 0; j < count; ++j)
...
int DLMS::GetActionInfo(DLMS_OBJECT_TYPE objectType, unsigned char& value, unsigned char& count)
{
    switch (objectType)
    {
    ...
    DLMSVariant& value)
{
    ...
    return Helpers::SetData(settings, bb, tp, value);
}

int DLMS::ParseSnrmUaResponse(
    ByteBuffer& data,
    HdlcSettings* limits)
{
    ...
    if (data.GetSize() == 0)
    {
        limits->SetMaxInfoRX(DLMSLimits::DEFAULT_MAX_INFO_RX);
        limits->SetMaxInfoTX(DLMSLimits::DEFAULT_MAX_INFO_TX);
        limits->SetWindowSizeRX(DLMSLimits::DEFAULT_WINDOWS_SIZE_RX);
        limits->SetWindowSizeTX(DLMSLimits::DEFAULT_WINDOWS_SIZE_TX);
        return 0;
    }
    // Skip FromatID
    if ((ret = data.GetUInt8(&ch)) != 0)
...
void DLMS::AppendHdlcParameter(ByteBuffer& data, unsigned short value)
{
    if (value < 0x100)
    {
        data.SetUInt8(1);
...
int DLMS::HandleConfirmedServiceError(ReplyData& data)
{
    int ret;
    unsigned char ch;
#ifndef DLMS_IGNORE_XML_TRANSLATOR
...
int DLMS::HandleExceptionResponse(ReplyData& data)
{
    int ret;
    unsigned char ch;
