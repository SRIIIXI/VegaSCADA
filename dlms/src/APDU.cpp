    ...
    bool ignoreAcse)
{
    ...
    if (settings.GetAuthentication() != DLMS_AUTHENTICATION_NONE)
    {
        ...
        ByteBuffer* callingAuthenticationValue;
        ...
        // Len
        Helpers::SetObjectCount(2 + callingAuthenticationValue->GetSize(), data);
        // Add authentication information.
        data.SetUInt8(BER_TYPE_CONTEXT);
        // Len.
        Helpers::SetObjectCount(callingAuthenticationValue->GetSize(), data);
    ...
    Cipher* cipher)
{
    ...
    if (settings.GetProtocolVersion() != NULL)
    {
        ...
        DLMSVariant tmp = settings.GetProtocolVersion();
        Helpers::SetBitString(data, tmp, false);
    }
        ...
        (ciphered || settings.GetAuthentication() == DLMS_AUTHENTICATION_HIGH_GMAC))
    {
        ...
        // LEN
        Helpers::SetObjectCount(2 + cipher->GetSystemTitle().GetSize(), data);
        data.SetUInt8(BER_TYPE_OCTET_STRING);
        // LEN
        Helpers::SetObjectCount(cipher->GetSystemTitle().GetSize(), data);
...
int GetConformanceFromArray(ByteBuffer& data, unsigned int& value)
{
    int ret;
    ...
    {
        value = Helpers::SwapBits(v1);
        value |= Helpers::SwapBits(v2) << 8;
        value |= Helpers::SwapBits(v3) << 16;
    }
    return ret;
}

// Reserved for internal use.
int SetConformanceToArray(unsigned int value, ByteBuffer& data)
{
    int ret;
    if ((ret = data.SetUInt8(Helpers::SwapBits((unsigned char)(value & 0xFF)))) != 0 ||
        (ret = data.SetUInt8(Helpers::SwapBits((unsigned char)((value >> 8) & 0xFF)))) != 0 ||
        (ret = data.SetUInt8(Helpers::SwapBits((unsigned char)((value >> 16) & 0xFF)))) != 0)
    {
    ...
    Cipher* cipher, ByteBuffer& data)
{
    ...
    {
        data.SetUInt8(0x1);
        ByteBuffer& dedKey = settings.GetCipher()->GetDedicatedKey();
        Helpers::SetObjectCount(dedKey.GetSize(), data);
...
int APDU::GenerateUserInformation(
    DLMSSettings& settings,
    Cipher* cipher,
    ...
    ByteBuffer& data)
{
    ...
    else
    {
        ...
        else
        {
            ...
            ByteBuffer crypted;
            ByteBuffer& key = cipher->GetBlockCipherKey();
            ...
            // Length for AARQ user field
            Helpers::SetObjectCount(2 + crypted.GetSize(), data);
            // Coding the choice for user-information (Octet string, universal)
            data.SetUInt8(BER_TYPE_OCTET_STRING);
            Helpers::SetObjectCount(crypted.GetSize(), data);
            data.Set(&crypted);
        }
    }
    return 0;
...
void GetConformance(unsigned long value, DLMSTranslatorStructure* xml)
{
    std::string str;
    unsigned long tmp = 1;
...
int APDU::Parse(bool initiateRequest,
    DLMSSettings& settings,
    Cipher* cipher,
    ByteBuffer& data,
#ifndef DLMS_IGNORE_XML_TRANSLATOR
        ...
        tag == DLMS_COMMAND_GENERAL_DED_CIPHERING)
    {
        ...
        if (xml != NULL)
        {
            ...
            if (cipher != NULL && xml->GetComments())
            {
                ...
                str = "Security: ";
                str.append(DLMSConverter::ToString(security));
                xml->AppendLine(str);
...
int ParseProtocolVersion(DLMSSettings& settings,
    ByteBuffer& buff)
{
    ...
    ByteBuffer sb;
    Helpers::ToBitString(sb, value, 8 - unusedBits);
    ...
    while (buff.GetPosition() < buff.GetSize())
    {
        ...
        case BER_TYPE_CONTEXT | BER_TYPE_CONSTRUCTED | PDU_TYPE_CALLED_AP_TITLE:
            ...
            else
            {
                ...
                if (xml != NULL)
                {
                    ...
                    {
                        str = DLMSConverter::ToString(result);
        ...
        case BER_TYPE_CONTEXT | BER_TYPE_CONSTRUCTED | PDU_TYPE_CALLED_AE_QUALIFIER:
            ...
            else
            {
                ...
                if (xml != NULL)
                {
                    ...
                    {
                        str = DLMSConverter::ToString(diagnostic);
                        xml->AppendComment(str);
                    }
                    std::string str;
        ...
        case BER_TYPE_CONTEXT | PDU_TYPE_CALLING_AE_INVOCATION_ID:
            ...
            if (xml != NULL)
            {
                if (xml->GetOutputType() == DLMS_TRANSLATOR_OUTPUT_TYPE_SIMPLE_XML)
                {
                    str = DLMSConverter::ToString(settings.GetAuthentication());
                    xml->AppendLine((unsigned long)tag, "", str);
                }
                else
                {
                    str = Helpers::IntToString(settings.GetAuthentication());
                    xml->AppendLine((unsigned long)tag, "", str);
                }
            }
#endif //DLMS_IGNORE_XML_TRANSLATOR
    ...
    ByteBuffer* encryptedData)
{
    ...
    if (settings.GetAuthentication() > DLMS_AUTHENTICATION_LOW)
    {
        ...
        // Add tag.
        data.SetUInt8(0xAA);
        Helpers::SetObjectCount(2 + settings.GetStoCChallenge().GetSize(), data); // Len
        data.SetUInt8(BER_TYPE_CONTEXT);
        Helpers::SetObjectCount(settings.GetStoCChallenge().GetSize(), data);
    ...
    if (result == DLMS_ASSOCIATION_RESULT_ACCEPTED || cipher == NULL || cipher->GetSecurity() == DLMS_SECURITY_NONE)
    {
        ...
        if (encryptedData != NULL && encryptedData->GetSize() != 0)
        {
            ...
            Helpers::SetObjectCount(encryptedData->GetSize(), tmp);
        ...
        Helpers::SetObjectCount(2 + tmp.GetSize(), data);
        // Coding the choice for user-information (Octet STRING, universal)
        data.SetUInt8(BER_TYPE_OCTET_STRING);
        // Length
        Helpers::SetObjectCount(tmp.GetSize(), data);
        data.Set(&tmp);
    }
